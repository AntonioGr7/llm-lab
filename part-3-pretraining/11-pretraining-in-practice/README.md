...
Part 3 isn't over. The next module — [Module 12 — Production Data Pipelines](../12-production-data-pipelines/) — fixes the two things this module's streaming loader can't do: checkpoint its data position and tokenize only once. Then [Module 13 — Continual Pretraining](../13-continual-pretraining/) closes Part 3 by showing how to keep teaching a *finished* base model genuinely new knowledge — the technique you reach for to internalize a private corpus no public model has ever seen. After that, [Part 4 — Post-Training](../../part-4-post-training/) opens with the post-training landscape, then SFT, preference optimization (DPO/IPO/KTO), reasoning (GRPO), and distillation. The base model you produce here feeds all of it — though for cost reasons Part 4 switches to **Qwen3-1.7B** as the post-training base, so those techniques are demonstrated on a more capable model than this 150M demo.
