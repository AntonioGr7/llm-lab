# Module 20 — Evaluation That Actually Means Something
...
The course built a model through stages: base → CPT (M13) → SFT (M15) → DPO (M17) → GRPO (M18) / distillation (M19). Labs evaluate **at every stage**, because each stage has a different goal and a different failure mode, and because that's how you localize a regression.
...
| **DPO (M17)** | pairwise judge win-rate vs the SFT model | helpfulness, preference alignment | **reward hacking** — longer/sycophantic answers that judges love but users don't; **over-refusal** |
| **GRPO (M18)** | GSM8K/MATH avg@k + maj@k | reasoning, verifiable accuracy | **format collapse**, **prior-skill loss** on non-reasoning evals |
| **Distillation (M19)** | the two-axis (new skill + prior skill) | the target capability | **forgetting** the prior skill (the whole SDFT point) |
...
# Score a specific stage's checkpoint (e.g. the GRPO model from Module 18),
# GSM8K only, with pass@k from 8 samples.
python eval.py --config=configs/eval_qwen3_1.7b.yaml \
    --model.checkpoint=../18-reasoning-and-grpo/results/checkpoints/step_00000300 \
...
**Next:** Part 6 — the bigger picture. Module 21 (Reading the Literature) and Module 22 (Scaling Intuitions / Career). You now have the full loop: build a model, post-train it, and — the skill most people skip — *know whether it actually got better.*
