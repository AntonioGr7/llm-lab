"""Optimizer construction for Module 16 — LoRA / QLoRA.

Copied verbatim from Module 15. It needs no LoRA-specific changes because of
one line in `_build_adamw`: `if not p.requires_grad: continue`. After
`lora.inject_lora_adapters` freezes the base model and leaves only the adapter
A/B matrices trainable, that filter automatically builds optimizer groups over
*just the adapters* — a few MB of state instead of 2× the model size. The same
decay / no-decay split applies (adapters are 2D, so they land in the decay
group; LoRA typically runs with `weight_decay=0.0`, the default here).
