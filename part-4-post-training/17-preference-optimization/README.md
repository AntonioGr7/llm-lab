# Module 17 — Preference Optimization (DPO / IPO)
...
Like Module 15, this is a **framework directory** — lift it out, point it at your own SFT checkpoint and preference data, you have a working DPO codebase. Modules 18-19 (GRPO, distillation) build on this same dual-model machinery.
...
[Module 18 — Reasoning and GRPO](../18-reasoning-and-grpo/) — DPO teaches the model *which completion is better* among ones it can already write. GRPO teaches it to *generate completions it couldn't write before* by rewarding verified-correct outcomes (math, code). Same dual-model machinery (policy + reference + KL anchor); a new loss + actual on-policy rollouts.
