# Module 19 — Distillation

You have a frontier model, or a teacher you trust, or just a model that works *and* a small set of demonstrations of a new behavior. You want a smaller, cheaper, or differently-skilled student. Distillation is how you move capability through token-level supervision instead of through outcome reward (Module 18) or pairwise preference (Module 17).
...
├── fsdp_setup.py          copied from Module 18
├── optim.py               copied from Module 18
├── schedule.py            copied from Module 18
├── checkpoint.py          copied from Module 18
├── efficiency.py          copied from Module 18
├── configs/
...
NEW components vs Module 18: `prepare_tooluse_corpus.py` (real-data loader), `make_tooluse_corpus.py` (synthetic fallback), the forward-KL `loop.py`, and a much-simplified `rollout.py` (one rollout per prompt; no group sampling). Infra (`optim.py`, `schedule.py`, `fsdp_setup.py`, `checkpoint.py`, `efficiency.py`) is identical to M18.
...
The cost: **two models in memory**. The teacher is BF16 + no_grad (same memory shape as M17/M18's reference model), so the overhead is ~3.4 GB at 1.7B teacher — manageable on A100.
...
3. **Demonstrations replace rewards.** Module 18's GRPO needed verifiable rewards (math correctness, code tests). SDFT needs only K worked examples of the target behavior. For domains where verification is hard (writing in a brand voice, following a domain-specific format, learned-from-PDF Q&A), this is a substantial unlock.
...
| A reward function (math correctness, test pass rate) | A model that gets better at outcomes you can verify | Not distillation. **Use [Module 18](../18-reasoning-and-grpo/) GRPO.** |
...
- **Distillation + GRPO.** Run R1-Distill style SFT first (Module 15 with teacher data), then GRPO (Module 18) on a reward signal. The DeepSeek-R1-Distill series uses exactly this recipe.
