# Module 12 — The Post-Training Landscape
...
Post-training is how you fix the product without ruining the function. This module is the map of how labs do it in 2026. The next four modules (13-16) are where you actually run each technique on a real model — Qwen3-1.7B on a single A100, every algorithm in the modern stack, every script torchrun-ready.
...
The fourth lever — **distillation** — is orthogonal: it's how you move any of these behaviors from a big model into a small one (Module 16).
...
**Distillation can replace any of these.** If a bigger lab has already done stages 1-3, you can often skip ahead by training your model to imitate theirs. R1-Distill is the cleanest example: take Qwen2.5 base, do SFT on R1's outputs, get most of R1's reasoning at a fraction of the compute. Module 16 covers three flavors of this.

**Safety/red-teaming is a parallel track.** Not shown in the diagram because it's not a separate stage — it's *data and reward signals injected into every stage above*. Refusal training is in the SFT data. Harm-avoidance preferences are in the DPO data. Constitutional/RLAIF approaches generate this data at scale. We touch on it in Module 14; a serious treatment is outside the course scope.
...
3. **LoRA / parameter-efficient methods.** If you only update a low-rank adapter, you can't move the base weights very far. The base capability is preserved by construction. Module 13 covers this.
4. **SDFT (self-distillation FT).** The technique from Shenfeld et al. (2026) we cover in Module 16. The same model acts as its own teacher on demonstrations, which dramatically reduces the gradient updates that erase prior capability.
...
**Synthetic with verification.** For math, code, and any domain with a checkable answer: generate candidates, filter by a verifier. The verifier can be a test harness (code), a calculator (math), or another LLM (everything else). This is how GRPO datasets get built; Module 15 walks through it.
...
A practical lesson: **before you write a single line of training code for post-training, you should know exactly what dataset you're going to use, where it came from, and what failure mode it's targeting.** Modules 13-16 each frame their algorithm around a specific dataset. That's deliberate.
...
|--------------------------------------------|-----------------------------|----------------------------------------------------------------------|
| Doesn't follow instructions / wrong format | **SFT** (Module 13)         | Format is teachable from demonstrations alone.                       |
| Gives the wrong answer style / tone        | **DPO** (Module 14)         | Style is a ranking problem, not a demonstration problem.             |
| Won't refuse harmful requests              | **DPO** with safety pairs   | Same. Mix safety prefs into the same DPO run.                        |
| Can't do multi-step reasoning              | **GRPO** (Module 15)        | Reasoning needs verification signal; demonstrations alone leak it.   |
| Hallucinates facts                         | **RAG first, then SFT**     | This is mostly not a training problem. Train only if RAG can't fix.  |
| Model is too big to serve cheaply          | **Distillation** (Module 16)| Compress capability into a smaller student.                          |
| Lost prior skill after fine-tuning         | **SDFT** (Module 16) / LoRA | Both reduce gradient damage to base weights.                         |
...
- **Constitutional AI and RLAIF in depth.** The principle (use an LLM as the preference labeler) shows up in Module 14 as a data-generation technique. The full Anthropic methodology — multi-step critique-and-revise pipelines, principle-based constitutions — is its own field. Start with the Bai et al. (2022) paper.
...
**The recent frontier:**
- Shenfeld et al., *Self-Distillation Fine-Tuning* (2026). The paper we implement in Module 16.
...
Once you've seen the gap, structure, and tax with your own eyes, you're ready for Module 13 — where we close the format gap with SFT.
