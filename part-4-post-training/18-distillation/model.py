"""Model builders for Module 18 — Distillation.
...
model-loading requirements:
  ...
  - **on_policy**: TWO models. Student is FP32-master, trainable, FSDP.
                   ...
                   model. Same dual-model pattern as Modules 16/17.
...
The `generation_mode` context manager (also in Module 17) flips KV
...
def build_student(cfg: ModelConfig) -> nn.Module:
    ...
    Identical recipe to Modules 16/17 build_policy: FP32 master, no KV
...
def build_teacher(cfg: ModelConfig) -> nn.Module:
    ...
    Same recipe as Module 16's build_reference / Module 17's
...
def generation_mode(model: nn.Module):
    ...
    Same helper as Module 17's. Critically reused here for SDFT: the
...
def count_params(model: nn.Module) -> dict[str, int]:
    """Param-count breakdown by group. Carried over from Module 17."""
