"""The Module 18 distillation entrypoint. Launch via `torchrun`.
...
def _parse_args(argv):
    p = argparse.ArgumentParser(
        description="Module 18 Distillation entrypoint (SDFT + on-policy)",
...
def main(argv=None):
    ...
    if cfg.distill.mode == "offline":
        raise SystemExit(
            "Offline distillation is README-only in Module 18 (the loss is "
