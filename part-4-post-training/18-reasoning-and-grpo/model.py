"""Model builders for Module 18 — Reasoning and GRPO.

Identical pattern to Module 17: build_policy + build_reference. Two models,
one trainable (FP32 master, BF16 compute via FSDP MixedPrecision) and one
frozen (BF16, no grad, eval mode).

What's new vs Module 17 is the *use*: the policy is asked to GENERATE in
...
def build_policy(cfg: ModelConfig) -> nn.Module:
    ...
    Identical to Module 17's `build_policy`: FP32 master weights, no KV cache,
...
def build_reference(cfg: ModelConfig) -> nn.Module:
    ...
    Same recipe as Module 17. The reference contributes the per-token KL
